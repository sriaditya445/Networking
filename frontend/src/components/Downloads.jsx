...
import { FaDownload, FaFolder, FaFileArchive, FaSpinner } from 'react-icons/fa';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {displayJobs.map((job) => {
            ...
            if (isProcessing) {
              ...
              statusBadge = (
                <span className="text-rose-500 font-bold uppercase text-[9px] tracking-wide bg-rose-50 px-2 py-0.5 rounded border border-rose-100">
                  Failed
                </span>
              );
            }
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-slate-650 flex justify-between items-center mt-3 pt-3 border-t border-slate-100 font-mono">
                  <span>Files: {job.total_devices || job.files_count || 0}</span>
                  {statusBadge}
                </div>
                ...
                <button
                  ...
                  className={`w-full font-semibold py-2 rounded-lg text-xs mt-3 flex items-center justify-center gap-1.5 transition-colors
                    ${isProcessing || isFailed
                      ? 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200'
                      : 'bg-slate-900 hover:bg-cyan-500 hover:text-slate-950 text-white cursor-pointer'
                    }`}
                >
