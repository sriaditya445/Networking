...
function Inventory({ devices = [], jobs = [], onTypeClick }) {
  ...
  const components = [
    {
      ...
      gradient: 'from-cyan-500/10 to-teal-500/5 hover:border-cyan-400/40',
      icon: <FaNetworkWired className="text-4xl text-cyan-500" />
    },
    {
      id: 'L3 Switch',
      title: 'L3 Switches',
      count: counts['L3 Switch'] || 0,
      status: 'Operational',
      badgeColor: 'bg-blue-50 text-blue-600 border-blue-100',
      description: 'Layer 3 distribution switches performing inter-VLAN routing and local interface gateway bindings.',
      gradient: 'from-blue-500/10 to-indigo-500/5 hover:border-blue-400/40',
      icon: <FaNetworkWired className="text-4xl text-blue-500" />
    },
    {
      id: 'Core Switch',
      title: 'Core Switches',
      count: counts['Core Switch'] || 0,
      status: 'Operational',
    ...
    },
    {
      ...
      gradient: 'from-amber-500/10 to-yellow-500/5 hover:border-amber-400/40',
      icon: <FaServer className="text-4xl text-amber-500" />
    },
    {
      id: 'Unknown',
      ...
      gradient: 'from-slate-500/15 to-zinc-500/5 hover:border-slate-400/40',
      icon: <FaQuestionCircle className="text-4xl text-slate-500" />
    }
  ];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Title */}
      <div>
        <h2 className="text-xl font-bold text-slate-800">Network Inventory</h2>
      ...
      {/* Folder Filter Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex items-center gap-3 shrink-0">
          <span className="text-xs text-slate-550 font-medium flex items-center gap-1.5 font-sans">
            <FaFilter className="text-slate-400 text-[10px]" />
            <span>Filter Inventory by Folder:</span>
          </span>
          <select
            className="bg-white border border-slate-200 rounded-xl py-1.5 pl-3 pr-8 text-xs text-slate-800 focus:outline-none focus:border-cyan-500 cursor-pointer font-bold"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {components.map((comp) => (
          <div
            ...
            className={`group bg-white border border-slate-100 rounded-3xl p-5 flex flex-col justify-between h-64 shadow-sm hover:shadow-md cursor-pointer transition-all duration-300 hover:-translate-y-1.5 relative overflow-hidden bg-gradient-to-br ${comp.gradient}`}
          >
            ...
            <div className="space-y-1.5 max-w-[70%]">
              <span className={`inline-block px-2.5 py-0.5 border rounded-full text-[9px] font-bold tracking-wider uppercase ${comp.badgeColor}`}>
                {comp.status}
              </span>
            ...
            <div className="flex justify-between items-center border-t border-slate-100 pt-3 mt-4">
              <div className="flex flex-col">
                ...
                <span className="text-lg font-bold text-slate-800 font-mono mt-1">{comp.count}</span>
              </div>
              <span className="w-7 h-7 rounded-full bg-slate-50 text-slate-400 group-hover:bg-slate-100 group-hover:text-slate-700 flex items-center justify-center transition-all duration-300 shadow-sm border border-slate-200/40">
